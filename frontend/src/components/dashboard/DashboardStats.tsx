...
export function DashboardStats({ stats }: DashboardStatsProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {/* Total Students Card */}
      <div className="bg-card border border-divider rounded-[2px] p-4 sm:p-5 shadow-2xs flex flex-col justify-between">
      ...
      <div className="bg-card border border-divider rounded-[2px] p-4 sm:p-5 shadow-2xs flex flex-col justify-between border-l-4 border-l-[#B23B3B]">
        <div className="flex items-center justify-between pb-2 border-b border-divider mb-3">
          <span className="text-xs font-mono font-bold uppercase text-[#B23B3B] tracking-wider">
            SABABSIZ YO&apos;Q
          </span>
          <span className="w-2.5 h-2.5 rounded-full inline-block animate-pulse" style={{ backgroundColor: "var(--status-unexcused)" }} />
        </div>
        <div className="flex items-baseline justify-between">
          <span className="font-mono text-3xl font-bold text-[#B23B3B] tracking-tight">
